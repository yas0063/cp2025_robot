...
// 半径の小さい順に物体番号をバブルソート
void radius_babble_sort_number(Object *object) {
  int i, j;
  Object temp;

  for (i = 0; i < OBJECT_NUM; i++) {
    for (j = OBJECT_NUM - 1; j > i; j--) {
      if (object[j - 1].radius > object[j].radius) {
        temp = object[j];
        object[j] = object[j - 1];
        object[j - 1] = temp;
      }
...
void object_move(Object *object, int n, Position target_pos) {
  ...
  switch (task_state) {
    ...
    case SORT:
      radius_babble_sort_number(object);
      task_state = APPROACH;
...
int main(void) {
  ...
  Position target_pos = {0.0, 0.7};  // 物体の移動先
    ...
    if (get_arm_state() == ARM_STATE_STOP) {
      ...
      if (n < OBJECT_NUM) {
        if (task_state != FINISH) {
          ...
          // 物体の設置に完了したら次の物体へ
          target_pos.y -= object[n].radius + object[n + 1].radius + 0.05;
          task_state = APPROACH;
          n++;
        }
